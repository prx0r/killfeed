...
Stockify is a **personal signal compiler**:
...
- Standalone Stockify MCP server with `list_feeds`, `get_feed`, `search_feed`.
...
cp .env.example .env
uv sync
uv run python -m stockify.cli seed
uv run uvicorn stockify.api:app --reload --port 8000
```
...
```bash
uv run python -m stockify.cli ingest github hackernews glama --limit 20
```
...
TrustMRR, Store Leads, and Apify expose remote MCP surfaces. With the MCP extra installed, Stockify can introspect/call Streamable-HTTP MCP servers:

```bash
uv sync --extra mcp
```

Stockify automatically registers sensible MCP presets:
...
Open any feed in Safari, e.g. `/f/agent-commerce-alpha`, then use **Share → Add to Home Screen**. Stockify emits a feed-specific web manifest, `apple-mobile-web-app-title`, and generated icons. Each feed therefore gets a distinct launcher while sharing one underlying engine.

This is separate from a future native Stockify client; the backend contracts are already suitable for one.
...
stockify/
  ...
  mcp_server.py         Stockify as an MCP server
...
Do **not** turn Stockify into another generic social reader. Every new feature should improve one of:
