...
from stockify.models import IngestionRun, Signal, SourceRecord
    ...
    try:
        async with adapter_cls() as adapter:
            if not adapter.configured:
                ...
                session.commit()
                return run
            items = await adapter.fetch(limit or settings.stockify_ingest_limit)
        inserted = 0
        signals_created = 0
        for item in items:
